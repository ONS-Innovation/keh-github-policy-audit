...
run "dev_resource_naming" {
  ...
  assert {
    condition     = aws_cloudwatch_event_rule.weekly_audit["ONS-Innovation"].name == "sdp-dev-github-policy-audit-ONS-Innovation-weekly-trigger"
    error_message = "EventBridge rule name should be sdp-dev-github-policy-audit-ONS-Innovation-weekly-trigger."
  }
