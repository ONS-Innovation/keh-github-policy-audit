"""Unit tests for dependabot repository check handler."""

import importlib
from unittest.mock import create_autospec, patch

module = importlib.import_module("functions.repository_checks.dependabot.handler")
...
class TestDependabotHandler:
    def test_wire_client_and_repo(self) -> None:
        ...
        mock_check = create_autospec(module.check_dependabot, side_effect=fake_check)

        with (
            patch("utils.github.get_github_client", return_value=client),
            patch.object(module, "check_dependabot", mock_check),
        ):
        ...
        assert result == {"status": "PASS", "check_name": "dependabot"}
