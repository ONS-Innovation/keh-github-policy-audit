"""Unit tests for external_pull_request repository check handler."""
...
class TestExternalPullRequestHandler:
    def test_wire_client_and_repo(self) -> None:
        ...
        mock_check = create_autospec(
            module.check_external_pull_request, side_effect=fake_check
        )

        with (
            patch("utils.github.get_github_client", return_value=client),
            patch.object(module, "check_external_pull_request", mock_check),
        ):
        ...
        assert result == {"status": "PASS", "check_name": "external_pull_request"}
