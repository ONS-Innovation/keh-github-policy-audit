"""Unit tests for repository_access repository check handler."""
...
class TestRepositoryAccessHandler:
    def test_wire_client_and_repo(self) -> None:
        ...
        mock_check = create_autospec(
            module.check_repository_access, side_effect=fake_check
        )

        with (
            patch("utils.github.get_github_client", return_value=client),
            patch.object(module, "check_repository_access", mock_check),
        ):
        ...
        assert result == {"status": "PASS", "check_name": "repository_access"}
