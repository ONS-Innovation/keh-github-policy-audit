"""Unit tests for readme repository check handler."""

import importlib
from unittest.mock import create_autospec, patch

module = importlib.import_module("functions.repository_checks.readme.handler")
...
class TestReadmeHandler:
    def test_wire_client_and_repo(self) -> None:
        ...
        mock_check = create_autospec(module.check_readme, side_effect=fake_check)

        with (
            patch("utils.github.get_github_client", return_value=client),
            patch.object(module, "check_readme", mock_check),
        ):
        ...
        assert result == {"status": "PASS", "check_name": "readme"}
